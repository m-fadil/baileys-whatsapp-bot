import { encrypt, signature, decrypt } from './utils/crypt';
...
export class ApiGetContact {
    private _token: string;
    private _finalKey: string;
    ...
    async checkNumber(number: string) {
        try {
            ...
            const st = signature(ts, JSON.stringify(p), AWAS_BINTITAN.replace(AWAS_BINTITAN.substring(128), ''));

            const res = await request(AWAS_BINTITAN.substring(128), encrypt(JSON.stringify(p), this._finalKey), {
                'X-Os': 'android 9',
            ...
            const jsonRes: IJsonRes | undefined = JSON.parse(decrypt(res?.data?.data, this._finalKey));
            return {
                number,
            ...
            const errMsg = encryptedErr ? decrypt(encryptedErr, this._finalKey) : error.message || 'Something went wrong';
